...
* **D-029 Render loop (REPLACED):** High-frequency `after()` loop driving delta-time (`dt`) updates for the Sprite entities, completely decoupled from logical algorithm progression.
