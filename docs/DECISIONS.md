...
* **D-039 Insertion Sort tick sequence (REVISED — locked):** Each outer loop pass is bounded by two events: the key-selection T1 tick (first event — lifts the key to the compare lane) and the T2 Placement tick (final event — settles the key into its sorted position). Between these bookends, each shift requires both a T1 compare tick and a T2 shift tick — elements shift one at a time, never as a batch (see D-060). A terminating T1 compare is emitted when the loop exits by condition (not by `j < 0`). The key sprite remains elevated across all ticks until the T2 Placement settle. See `05_ALGORITHMS_VIS_SPEC.md` Section 4.3, `10_ANIMATION_SPEC.md` Section 5.2.
