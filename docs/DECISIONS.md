...
* **D-039 Insertion Sort tick sequence (REVISED — locked):** Each outer loop pass is bounded by two events: the key-selection T1 tick (first event — lifts the key to the compare lane) and the T2 Placement tick (final event — settles the key into its sorted position). Between these bookends, each shift requires both a T1 compare tick and a T2 shift tick — elements shift one at a time, never as a batch (see D-060, D-064). A terminating T1 compare is emitted when the loop exits by condition (not by `j < 0`). **Sustained elevation invariant:** the key sprite must maintain its elevated vertical position (`home_y - lift_offset`) from the moment the key-selection T1 tick fires until the final T2 Placement tick of the pass is received — it must never return to baseline between ticks within a pass. The T2 Placement tick triggers the diagonal settle (simultaneous x and y interpolation) back to `home_y`. See `05_ALGORITHMS_VIS_SPEC.md` Section 4.3, `10_ANIMATION_SPEC.md` Section 5.2.
