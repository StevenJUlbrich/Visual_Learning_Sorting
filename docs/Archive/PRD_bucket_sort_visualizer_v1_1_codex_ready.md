...
## Bucket Sort Visualizer — Lightning Labs Style (PRD v1.1, Codex-ready)
...
## 1.5 Working with Codex (current)

This PRD is written to be machine-actionable by **Codex** (Codex CLI / Codex App / IDE extension). To make it work smoothly with the current Codex instruction system:

### 1.5.1 Required repo docs layout (P0)

**Requirement:** The repository MUST contain these files at the repo root:

- `AGENTS.md` — persistent instructions for Codex
- `PRD.md` — this document (keep the filename stable)
- `DESIGN.md` — design/animation reference notes (from Design Approach v3)
- `PLANS.md` — optional; used for longer “ExecPlan” work (see below)

Codex reads `AGENTS.md` **before doing any work** and uses it to load project expectations and guardrails.

### 1.5.2 AGENTS.md scope guidance (P0)

**Requirement:** `AGENTS.md` MUST:

- Point Codex to `PRD.md` as the source of truth (requirement IDs are binding)
- Define **definition of done**: lint + tests + manual verification steps
- Define how to run the project (Windows / PowerShell commands)
- Define whether duplicates are allowed (or require stable element IDs)

**Note:** Keep `AGENTS.md` concise. If it grows large, add nested `AGENTS.md` / `AGENTS.override.md` files in subfolders (e.g., `src/`, `tests/`) rather than one huge file.

### 1.5.3 When to use PLANS.md (P1)

For multi-hour epics or complex refactors, Codex performs best with a “living” execution plan (ExecPlan). If enabled, add an ExecPlan section to `PLANS.md` and keep the progress checklist updated as work proceeds.

### 1.5.4 Codex workflow recommendation (P0)

**Requirement:** Engineering work MUST be sliced into small, testable milestones aligned to PRD requirement IDs (e.g., “Implement PRD-4.2 SCATTER arc + stagger + code sync”).

**Recommended prompt pattern:**
- “Implement PRD-4.2.* only; do not start PRD-4.3; run tests; summarize changes and where to review.”
