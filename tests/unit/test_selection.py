"""Unit tests for SelectionSort generator — TC-A1, TC-A2, TC-A3, TC-A10."""
...
def test_tc_a1_final_sortedness(default_7: list[int]) -> None:
    sorter = SelectionSort(default_7)
...
def test_tc_a10_counter_accuracy(default_7: list[int]) -> None:
    ...
    assert sorter.comparisons == 21
    assert sorter.writes == 10


@pytest.mark.unit
def test_single_element_terminal_only(single_1: list[int]) -> None:
    sorter = SelectionSort(single_1)
    ticks = list(sorter.sort_generator())
    assert len(ticks) == 1
...
@pytest.mark.unit
def test_swap_skipped_when_already_sorted(sorted_7: list[int]) -> None:
    sorter = SelectionSort(sorted_7)
    ticks = list(sorter.sort_generator())
    swap_ticks = [t for t in ticks if t.operation_type is OpType.SWAP]
    assert len(swap_ticks) == 0
    assert sorter.writes == 0
